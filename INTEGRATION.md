...
Поток аутентификации извлекает идентичность подписанта из сертификата и сначала ищет пользователя по настраиваемой колонке:
...
Если выбранного поля нет в сертификате или таблице, безопасная цепочка fallback — колонки `pinfl` → `tin`/`inn` → `eimzo_serial_number`/`serial_number`. Колонка `inn` получает значение TIN сертификата, а `eimzo_serial_number` — его serial. `uid` не используется неявно, потому что это отдельный идентификатор сертификата; при необходимости укажите `EIMZO_USER_LOOKUP_COLUMN=uid` явно.
