...
- `frontend_url` — опциональный same-origin-алиас, который Java-сервис использует для эндпоинтов, видимых также и из браузера. Если задан относительный путь вроде `/frontend`, клиент перепишет его в `<схема запроса>://<хост>/frontend/...`, и тогда переиспользуется браузерный прокси. Если задан абсолютный — используется как есть.
